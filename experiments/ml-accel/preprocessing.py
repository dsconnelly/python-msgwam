...
def save_indices() -> None:
    """
    Save indices for training, validation, and test sets. Makes sure that held-
    out data includes novel wind profiles as well as novel ray volume data.
    """

    n_nontest = int(0.3 * N_BATCHES)
    n_nontest = n_nontest + n_nontest % 2
    a = N_BATCHES - n_nontest
    b = a + n_nontest // 2

    a, b = a * PACKETS_PER_BATCH, b * PACKETS_PER_BATCH
    idx = torch.arange(N_BATCHES * PACKETS_PER_BATCH)

    torch.save(idx[:a], f'{DATA_DIR}/idx-tr.pkl')
    torch.save(idx[a:b], f'{DATA_DIR}/idx-va.pkl')
    torch.save(idx[b:], f'{DATA_DIR}/idx-te.pkl')

def save_training_inputs() -> None:
    """
    Save zonal wind time series paired with fine and coarse ray volume data. See
    the specific generation functions for more details.
    """
...
def save_training_outputs() -> None:
    """
    Save momentum flux profiles corresponding to each fine and coarse ray
    volume generated by `save_training_inputs`. This function works with Slurm
    to split the integration over many tasks, if called in the context of a job
    array. Otherwise, all profiles are integrated in series.
    """

    wind = torch.load(f'{DATA_DIR}/wind.pkl')
    X = torch.load(f'{DATA_DIR}/X.pkl')
    ...
    if n_tasks == 1:
        Z_fine = _integrate_packets(wind, X, RAYS_PER_PACKET)
        Z_coarse = _integrate_packets(wind, Y, 1)

        # torch.save(Z_fine, f'{DATA_DIR}/Z-fine.pkl')
        # torch.save(Z_coarse, f'{DATA_DIR}/Z-coarse.pkl')

        torch.save(Z_fine, f'{DATA_DIR}/Z-fine-series.pkl')
        torch.save(Z_coarse, f'{DATA_DIR}/Z-coarse-series.pkl')

    else:
        n_per_task = int(2 * N_BATCHES // n_tasks)
        step = params.task_id % int(n_tasks // 2)

        tasks = [(X, 'fine', RAYS_PER_PACKET), (Y, 'coarse', 1)]
        data, suffix, per_packet = tasks[params.task_id >= n_tasks // 2]
        start, end = step * n_per_task, (step + 1) * n_per_task

        print(f'Job {params.task_id} integrating {suffix} data from {start} to {end}')

        Z = _integrate_packets(wind[start:end], data[start:end], per_packet)
        torch.save(Z, f'{DATA_DIR}/Z-{suffix}-{step}.pkl')

def stack_training_outputs() -> None:
    """
    In the event that the training outputs were integrated in parallel, this
    function stacks them in the correct order and saves the complete tensors.
    """
    ...
    for resolution in ['fine', 'coarse']:
        ...
        for fname in os.listdir(DATA_DIR):
            if not fname.startswith(f'Z-{resolution}-'):
                continue
        ...
        Z = torch.vstack([datas[i] for i in idx.tolist()])
        torch.save(Z, f'{DATA_DIR}/Z-{resolution}.pkl')
...
def _integrate_packets(
    ...
    Z = torch.zeros((n_batches, PACKETS_PER_BATCH, config.n_t_max, n_z))
    T = config.dt * config.n_t_max

    for i in range(n_batches):
        spectrum = X[i].reshape(9, -1)
        # Z[i] = config.dt * integrate_batch(
        #     wind[i],
        #     spectrum, 
        #     rays_per_packet,
        #     smoothing=None
